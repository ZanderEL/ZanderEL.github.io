...
"""
ANALYSIS: TOP 10 VS. TOP 15 WORD FREQUENCY COMPARISON

Top 10 vs. Top 15 Comparison:
The basic get_word_counts() approach produces a top 10 dominated by common English words 
such as "the", "and", "to", "of", and "a". In contrast, the spaCy-normalized approach 
yields a top 15 that reveals content-specific terms like "elizabeth", "darcy", "bennet", 
and "mr". The key difference is that basic tokenization treats all words equally, while 
preprocessing filters meaningless vocabulary.

Impact of Text Preprocessing:
The divergence stems from spaCy's preprocessing pipeline: it removes stop words (common 
function words like "the", "and"), eliminates punctuation, handles lemmatization (reducing 
words to their base form), and filters tokens shorter than 3 characters. These steps shift 
focus from high-frequency noise to semantically meaningful content. While the basic approach 
is simpler and faster, spaCy's preprocessing provides more actionable insights by exposing 
the text's actual substance rather than its grammatical scaffolding.

Insights About the Text:
The cleaned top 15 clearly reveals Pride and Prejudice's core subject matter: character 
names (Elizabeth, Darcy, Bennet, Jane) dominate, alongside socially relevant terms like 
"lady", "good", and "friend". This demonstrates how removing stop words uncovers the text's 
thematic essence—personal relationships and social dynamics. The preprocessing approach 
proves more valuable for content understanding, as it enables analysts to focus on 
significant semantic signals while eliminating repetitive "noise" that obscures meaning.
"""